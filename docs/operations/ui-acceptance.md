...
- 投稿フォームは提出者情報、作品情報、YouTube URL、確認・送信の順に進み、戻る操作とエラー箇所への復帰ができる。枠提出では YouTube URL は既定任意で、未入力でも次へ進める。イベントが YouTube URL を必須指定した場合は、枠提出でも未入力では次へ進めない。YouTube URL stepから確認 stepへ進む操作では送信せず、確認画面の明示的な提出操作だけが送信を開始する。自動送信を持つフォームは、`requestSubmit` 非対応ブラウザでも制約検証を維持して送信する。編集提出とウィザード確認提出は、イベント必須項目の欠落をクライアントでも拒否する。
- custom answerと合作memberの進捗が現在値から更新される。
- 共同編集権限の付与・解除は、確認ボタンの連打でも同時に1件だけ送信する。通信例外は画面内に表示し、処理結果が不明な場合は最新の権限を確認してから再試行する。保存後の一覧更新に失敗しても保存成功の表示は維持する。
...
- イベント運営画面はイベント色を概要ヘッダー・運営タブ・状態カードへ反映し、対応待ち・枠の進行状況・折りたたみ履歴を同じ視覚階層で確認できる。概要には `manage-event-action-rail` を置き、審査・作品・枠・参加者・投稿必須項目への主要導線をまとめる。760px以下では2列、430px以下では1列に切り替え、ドロワーと主要操作が画面幅内に収まる。
- イベント設定の「投稿の必須項目」は `event.questions` 権限の Server Action で保存し、投稿・枠提出・作品編集のサーバー検証でも同じ必須を適用する。
- 設定の申請履歴では、完了した X ID 統合について期限内に差し戻し申請でき、pending の差し戻しは取り下げできる。
...
- `{{#members}}` ループはネストを許可しない。3重以上の場合も最外側の閉じタグまでを除去し、その前後の本文・正常なループは保持する。
