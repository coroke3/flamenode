...
test("parseBoundedPositiveInt closes invalid fallback and max values", () => {
  assert.equal(
    parseBoundedPositiveInt(null, Number.NaN, Number.NaN),
    1,
  );
  assert.equal(parseBoundedPositiveInt("8", 24, 0), 1);
});

test("service unavailable response always emits a positive Retry-After", () => {
  assert.equal(
    publicServiceUnavailableResponse(
  ...
  assert.equal(
    publicServiceUnavailableResponse("database_unavailable", -10).headers.get(
      "Retry-After",
    ),
    "1",
  );
...
test("rate limiter uses connecting IP before forwarded IP and returns Retry-After", () => {
  ...
  assert.equal(
    checkPublicApiRateLimit(request("fallback"), endpoint, now),
    null,
  );
});
...
test("rate limiter cleans expired buckets and bounds key cardinality", () => {
  ...
  for (let i = 0; i < 2048; i++) {
    assert.equal(
      checkPublicApiRateLimit(request(`client-${i}`), endpoint, 2000),
      null,
    );
  }
  assert.equal(
    checkPublicApiRateLimit(request("overflow"), endpoint, 2000)?.status,
    429,
  );
  assert.equal(
    checkPublicApiRateLimit(request("expired"), endpoint, 2060),
    null,
  );
});
