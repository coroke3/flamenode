...
function cleanupRateLimitBuckets(now: number): void {
  for (const [key, bucket] of rateLimitBuckets) {
    if (now - bucket.windowStart >= RATE_LIMIT_WINDOW_SECONDS) rateLimitBuckets.delete(key);
  }
}

/** Isolate 単位の best-effort 固定窓制限。D1/KV write は行わず、分散 isolate 間では共有されない。 */
export function checkPublicApiRateLimit(request: Request, endpoint: string, now = Math.floor(Date.now() / 1000)): Response | null {
  cleanupRateLimitBuckets(now);
  const key = getClientKey(request, endpoint);
  let bucket = rateLimitBuckets.get(key);
  if (!bucket || now - bucket.windowStart >= RATE_LIMIT_WINDOW_SECONDS) {
    if (!bucket && rateLimitBuckets.size >= MAX_RATE_LIMIT_KEYS) return rateLimitedResponse(RATE_LIMIT_WINDOW_SECONDS);
    bucket = { windowStart: now, count: 0 };
    rateLimitBuckets.set(key, bucket);
  }
  bucket.count += 1;
  if (bucket.count <= RATE_LIMIT_MAX_REQUESTS) return null;
  return rateLimitedResponse(Math.max(1, RATE_LIMIT_WINDOW_SECONDS - (now - bucket.windowStart)));
}
...
async function payloadEtag(payload: unknown): Promise<string> {
