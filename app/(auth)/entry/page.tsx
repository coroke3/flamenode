...
export default async function EntryPage({
  ...
  const groupedReservedSlots: Array<SlotGroupRow & {
    event_entry_end_time: number | null;
  }> = collapseReservationGroups(reservedSlots).map((slot) => ({
    ...slot,
    event_entry_end_time:
      reservedSlots.find((row) => row.id === slot.id)?.event_entry_end_time ?? null,
  }));
  groupedReservedSlots.sort((a, b) => {
    ...
    if (aNeedsSubmission !== bNeedsSubmission) {
      return aNeedsSubmission ? -1 : 1;
    }
    const deadlineDiff =
