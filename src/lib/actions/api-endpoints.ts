...
async function setPublicApi(
  ...
  if (!eventId || eventId.length > 128) {
    return { ok: false, message: "event_idが不正です。" };
  }
  ...
  if (enabled === 1 && before.visibility_status !== "public") {
    return { ok: false, message: "公開イベントだけAPIを有効化できます。" };
  }
      ...
      audits: [
        {
          ...
          context: "admin_api_endpoints",
          reason: enabled ? "公開APIリンクを発行" : "公開APIを無効化",
...
export async function setApiEndpointActive(
  ...
  const enabled =
    String(formData.get("public_api_enabled") ?? "0") === "1" ? 1 : 0;
