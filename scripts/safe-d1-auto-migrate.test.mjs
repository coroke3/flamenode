...
test("production deploy applies guarded migrations before strict schema preflight and workers", () =>
...
test("migration apply failure stops strict preflight and every worker deploy", () =>
  withTempDirectory("flamenode-deploy-safe-migration-failure-", (repoRoot) => {
    ...
    fs.writeFileSync(fakeWrangler, "// fixture\n", "utf8");
    const workerDeploys = [];
    let strictPreflightReached = false;
    assert.throws(
      () =>
        deployProduction({
          env: {
          ...
          schemaPreflight: () => {
            strictPreflightReached = true;
          },
          run: ({ label }) => workerDeploys.push(label),
        }),
      /fixture migration failure/,
    );
    assert.equal(strictPreflightReached, false);
