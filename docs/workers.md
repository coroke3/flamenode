...
| Discord Bot/Webhook | per-route limitは可変。`X-RateLimit-*`と`Retry-After`を正本にし、global limitは50 requests/sec | 1 Cron最大12 external requests。inline retryなし。DM channel IDをisolate/KVへ30日cacheし、通常配送を1 requestへ削減。401/403/404は無限再試行しない |
