...
test("pickHeroEventsは最大3件に制限する", () => {
  const events = [
    event("e1", "e1", { start_time: NOW + 1000 }),
    event("e2", "e2", { start_time: NOW + 2000 }),
    event("e3", "e3", { start_time: NOW + 3000 }),
    event("e4", "e4", { start_time: NOW + 4000 }),
    event("e5", "e5", { start_time: NOW + 5000 }),
  ];
...
test("pickHeroEventsは終了・点イベントを除外する", () => {
  const events = [
    event("active", "active", { start_time: NOW + 1000 }),
    event("ended", "ended", { start_time: NOW - 7200, end_time: NOW - 3600 }),
    event("point", "point", { start_time: NOW + 1000, end_time: null }),
  ];
  const heroes = pickHeroEvents(events, 3, NOW);
...
test("pickHeroEventsは1件と2件でも正しく返す", () => {
  ...
  const two = pickHeroEvents(
    [event("a"), event("b", "b", { start_time: NOW + 5000 })],
    3,
    NOW,
  );
