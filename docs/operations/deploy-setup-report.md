...
> Verified against commit: `c7f71d90`
> Source of truth: [`DEPLOY.md`](../../DEPLOY.md)、[`workers.md`](workers.md)、[`LOCAL.md`](../../LOCAL.md)、`.dev.vars.example`、`wrangler.toml`、`workers/*/wrangler.toml`
...
- [ ] 初回は **workers.dev で smoke 成功後** にカスタムドメインと Discord を更新する（§8）。
- [ ] Remote D1 migration は本チェックリストの対象外とし、別途 [`migrations.md`](migrations.md) に従い手動適用する（§9）。
...
| 領域 | 名前 / 役割 |
| --- | --- |
| Web Worker | `flamenode-web`（OpenNext + Workers Static Assets） |
...
| 項目 | 確認内容 |
| --- | --- |
| `AUTH_URL` | Web の正式 HTTPS origin と一致（`NEXT_PUBLIC_SITE_URL` と同じ） |
| `AUTH_DISCORD_ID` | Discord Developer Portal の Client ID（Build Variable または Runtime Variable） |
...
- [ ] `AUTH_URL`・`NEXT_PUBLIC_SITE_URL`・Discord redirect の scheme / host / port が完全一致。
...
- [ ] **Runtime Variables** … 各 Worker の公開 metadata・origin・`BUILD_COMMIT_SHA`（§7）
...
`WORKERS_CI_COMMIT_SHA` は Workers Builds が自動提供する 40 桁 SHA を使用し、Dashboard で上書きしない。
...
公開 site metadata の既定値は wrangler template を正本とする。
...
3. [ ] 4 Worker の `workers.dev` URL を Build Variables（`FLAMENODE_WEB_URL` 等）へ登録
4. [ ] `flamenode-web` だけ Git 連携し、§4 の Build / Deploy command を保存
5. [ ] `main` へ push し、Workers Build を実行
6. [ ] deploy 固定順（Web → fast → content → sync）と production smoke が全項目成功
...
12. [ ] 旧 Pages からトラフィックを切り替え（削除は [`DEPLOY.md` §9](../../DEPLOY.md) の条件を満たしてから）
