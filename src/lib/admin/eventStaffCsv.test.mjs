...
test("buildEventStaffCsvPreview parses canonical 6-column format", () => {
  const preview = buildEventStaffCsvPreview({
    text: EVENT_STAFF_CSV_SAMPLE,
    existingSubjects: [{ x_user_id: "yamada", user_id: null }],
    isSiteAdmin: false,
  });
...
test("buildEventStaffCsvPreview rejects obsolete 5-column rows", () => {
  const preview = buildEventStaffCsvPreview({
  ...
  assert.match(preview.rows[0].errors.join(" / "), /6列/);
});
