...
test("limit/offsetは固定長の10進整数だけをbounded値として受け、limit=0は既定値へ戻す", () => {
  ...
  assert.match(route, /Number\.isSafeInteger\(parsed\) \|\| parsed < min/);
  assert.match(
    route,
    /url\.searchParams\.get\("limit"\),\s*DEFAULT_LIMIT,\s*1,\s*MAX_LIMIT/s,
  );
  assert.match(
    route,
    /url\.searchParams\.get\("offset"\),\s*0,\s*0,\s*MAX_OFFSET/s,
  );
  assert.doesNotMatch(route, /Math\.max\(\s*1,\s*parseBoundedDecimalParam/s);
