...
`top.json` は新着最大100件と、生成時点で公開から3年以上経過した作品を `scheduled_time` 昇順で最大200件プールし、生成時に Fisher-Yates シャッフル後 `nostalgic` 最大20件を抽出して保持する。トップ表示時だけおすすめ・新着・懐かし枠を並べ替え、JSON自体の順序は変更しない。新着 loop 棚はシャッフル元プール100件のうち最大40件だけをDOMへ載せ、3棚は1行の連続ループとし、流れる向きを左・右・左で交互にする。トップの hero 用 `slot_stats` は対象イベント最大3件に限定する。
