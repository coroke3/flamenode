...
|---|---|---|---|
| `flamenode-fast-jobs` | `0 * * * *` | `notification_outbox` 配信・スロット締切リマインド（`notification-dispatcher` 統合） | `DISCORD_BOT_TOKEN`、Forum Webhook 3種（すべて必須）。legacy `DISCORD_WEBHOOK_URL` は任意（[`workers.md`](./operations/workers.md)） |
