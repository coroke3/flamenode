...
export default async function AdminWorkersPage(): Promise<React.ReactElement> {
  ...
  return (
    <div>
      ...
      >
        <Link href="/admin/workers" className="fn-btn fn-btn-primary fn-btn-sm">
          最新状態へ更新
        </Link>
...
function MonitoringContent({
  ...
  const criticalJobs = snapshot.jobs.filter(
    (job) => job.level === "critical",
  ).length;
  const problemPipelines = snapshot.pipelines.filter(
    ...
    <>
      <section
        ...
        style={{
          ...
          borderWidth: 1,
          borderStyle: "solid",
          ...statusStyle(snapshot.overallLevel),
        }}
      >
        <div
          style={{
        ...
        >
          <div>
            ...
            <p style={{ margin: "4px 0 0", fontSize: 13 }}>
              {snapshot.overallMessage}
            </p>
          </div>
          <div style={{ fontSize: 12, fontVariantNumeric: "tabular-nums" }}>
            取得: {formatUnix(snapshot.generatedAt)}
          </div>
        </div>
      </section>
          ...
          <FnTable style={{ minWidth: 940 }}>
            ...
            <tbody>
              {snapshot.jobs.map((job) => (
                <WorkerRow key={job.jobName} job={job} />
              ))}
            </tbody>
          </FnTable>
        </div>
      ...
      <section style={{ marginTop: 24 }}>
        ...
        <p className="fn-muted fn-text-sm" style={{ margin: "0 0 10px" }}>
          新規流入が止まった場合の理論上の解消時間です。外部API障害やCloudflare制限は含みません。
        </p>
        <div style={{ overflowX: "auto" }}>
      ...
      <section style={{ marginTop: 24 }}>
        ...
        >
          {snapshot.artifacts.length === 0 ? (
            <div className="fn-card" style={{ padding: 14 }}>
              生成履歴がありません。
            </div>
          ) : (
              ...
              <div key={artifact.targetType} className="fn-card" style={{ padding: 14 }}>
                ...
                <div style={{ marginTop: 6, fontSize: 13 }}>
                  {artifact.generatedAt == null
                    ? "未生成"
                    : formatRelative(artifact.generatedAt)}
                </div>
                <div className="fn-muted fn-text-sm">
                  {formatUnix(artifact.generatedAt)}
                </div>
              </div>
            ))
          )}
      ...
      <section className="fn-card" style={{ marginTop: 24, padding: 16 }}>
        ...
        <p className="fn-muted fn-text-sm" style={{ margin: "0 0 12px" }}>
          CPU時間、exceededCpu、アカウント全体のD1日次使用量はアプリDBだけでは取得できません。Cloudflare Dashboardを正本として確認してください。
        </p>
        <div
          style={{
            display: "grid",
        ...
        >
          {PLATFORM_LIMITS.map((limit) => (
            ...
            >
              <div className="fn-muted fn-text-sm">{limit.label}</div>
  ...
  return (
    <tr>
      <td>
        ...
        <code className="fn-muted fn-text-sm">{job.jobName}</code>
        <div className="fn-muted fn-text-sm">{job.message}</div>
      </td>
      <td>
        <span className={`fn-badge ${badgeClass(job.level)}`}>
          {levelLabel(job.level)}
        </span>
      </td>
      <TimeCell value={job.lastStartedAt} />
      <TimeCell value={job.lastSucceededAt} />
      <TimeCell value={job.lastFailedAt} />
      <TimeCell value={job.nextExpectedAt} />
      <td style={{ maxWidth: 240, wordBreak: "break-word", fontSize: 12 }}>
        {job.lastErrorCode ?? "—"}
      </td>
      <td>
        <Link href={job.detailHref} className="fn-btn fn-btn-ghost fn-btn-sm">
          詳細
        </Link>
      </td>
    </tr>
...
function PipelineRow({
  ...
  return (
    <tr>
      <td style={{ fontWeight: 700 }}>{pipeline.label}</td>
      <td>
        <span className={`fn-badge ${badgeClass(pipeline.level)}`}>
          {levelLabel(pipeline.level)}
        </span>
      </td>
      <td style={{ fontVariantNumeric: "tabular-nums" }}>
        {formatCount(pipeline.backlog)}
      </td>
      <td style={{ fontVariantNumeric: "tabular-nums" }}>
        {formatCount(pipeline.capacityPerDay)}
      </td>
      <td>{durationLabel(pipeline.estimatedDrainMinutes)}</td>
      <td className="fn-muted fn-text-sm">{pipeline.note}</td>
      <td>
        <Link href={pipeline.detailHref} className="fn-btn fn-btn-ghost fn-btn-sm">
          管理
        </Link>
      </td>
    </tr>
...
function TimeCell({ value }: { value: number | null }): React.ReactElement {
  return (
    <td title={formatUnix(value)} style={{ fontSize: 12 }}>
      {value == null ? "—" : formatRelative(value)}
    </td>
  );
}
...
function StatCard({
  ...
  return (
    <div className="fn-card" style={{ padding: "12px 14px" }}>
      ...
      <div style={{ fontSize: 22, fontWeight: 800, wordBreak: "break-word" }}>
        {value}
      </div>
    </div>
  );
