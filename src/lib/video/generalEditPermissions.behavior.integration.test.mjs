...
test("updateVideo rejects profile_text change in normal mode", () => {
  assert.match(updateVideoSource, /changed\(parsedFormData\.profile_text, target\.creator_profile_text\)/);
  assert.match(updateVideoSource, /紹介文を編集する権限がありません。/);
});

test("normal mode compares submitter fields with the video snapshot, not x_users", () => {
  assert.doesNotMatch(updateVideoSource, /creatorXRow/);
  assert.match(updateVideoSource, /target\.creator_other_social_links/);
  assert.match(updateVideoSource, /target\.creator_youtube_channel_url/);
  assert.doesNotMatch(videoSavePlanSource, /buildSubmissionXUserPlan/);
});
