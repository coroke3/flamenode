...
> Verified against commit: `74acc6c`
...
- YouTube同期は50 IDを1 API requestにまとめる。失敗時retryを含めても最大2 request / 2 quota unitsとする。
...
Cloudflare Dashboardで`exceededCpu`、D1 rows read/written、Worker requests、queue滞留を確認する。YouTube quota error、Discord 429、画像proxyのfallback/stale増加もWorker logで確認する。CPU超過が継続するtargetは件数上限をさらに下げるかpayloadをページ分割する。Free枠で任意規模を無制限に保証することはできず、D1 500MB/databaseまたはCPU 10msが恒常的な制約になった段階でPaid移行を判断する。
