...
| `flamenode-fast-jobs` | `DISCORD_BOT_TOKEN`（必須）、`DISCORD_WEBHOOK_URL_FORUM_ACCOUNT` + `DISCORD_WEBHOOK_URL_FORUM_EVENT` + `DISCORD_WEBHOOK_URL_FORUM_SYSTEM`（3種すべて必須）。legacy `DISCORD_WEBHOOK_URL` は任意（`webhook_target` 無しの旧 pending 行のみ）。Forum ルーティングは [`docs/operations/workers.md`](./docs/operations/workers.md) |
