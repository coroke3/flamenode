...
- 2つが同一値なら重複排除し、1キーとして扱う。
- credential障害になったキーは6時間回避し、その間は正常なキーを先に利用する。6時間後に主キーを再確認する。
- 使用中キー、直近切替、障害種別は共有KVの`youtube-api:key-status:v1`へ秘密値を含めず記録し、`/admin/workers`で確認する。
- `quotaExceeded`、`dailyLimitExceeded`、`rateLimitExceeded`では副キーへ切り替えない。複数projectのquotaを合算・迂回する用途には使わず、quota不足時はGoogleへquota extensionを申請するか同期頻度を下げる。
- 429と5xxは既存どおり同じキーで最大2回まで再試行する。credential障害から副キーへ切り替わる場合でも、1 Cronの外部requestは最大3回に抑える。
...
- YouTube同期は50 IDを1 API requestにまとめる。通常は最大2 request、主キーのcredential障害から副キーへ切り替わる場合のみ最大3 requestとする。
