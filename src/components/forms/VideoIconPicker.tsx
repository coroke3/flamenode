...
export type VideoIconMode = "default" | "existing" | "upload" | "none";

/**
 * 作品ごとアイコンの選択コンポーネント。
 *
 * ユーザー既定アイコン (`x_users.icon_url`) を変更しない点が `XIdIconPicker` と異なる。
 * 出力は `<input type="hidden" name="icon_url">` と `icon_mode`。
 * 作品保存時に `videos.creator_icon_url` へそのまま入る。空文字なら null として保存される。
 */
export interface VideoIconPickerProps {
  candidates: string[];
  initialIconUrl?: string | null;
  /** 既定プロフィールのアイコン URL。icon_mode=default の判定に使う。 */
  defaultIconUrl?: string | null;
  disabled?: boolean;
  /** 親からの制御 (再適用ボタン等)。未指定なら内部 state。 */
  value?: string;
  onChange?: (url: string) => void;
}

function resolveIconMode(args: {
  selected: string;
  defaultIconUrl?: string | null;
  candidates: string[];
  uploadedUrls: ReadonlySet<string>;
}): VideoIconMode {
  const { selected, defaultIconUrl, candidates, uploadedUrls } = args;
  if (!selected) return "none";
  if (
    uploadedUrls.has(selected) ||
    selected.startsWith("/api/media/video-icons/")
  ) {
    return "upload";
  }
  if (defaultIconUrl && selected === defaultIconUrl) return "default";
  if (candidates.includes(selected)) return "existing";
  return "existing";
}
...
export function VideoIconPicker({
  ...
  const [uploadedUrls, setUploadedUrls] = React.useState<Set<string>>(() => new Set());

  React.useEffect(() => {
    if (controlledValue !== undefined) return;
    setInternalSelected(initialIconUrl ?? "");
  }, [controlledValue, initialIconUrl]);

  const iconMode = React.useMemo(
    () =>
      resolveIconMode({
        selected,
        defaultIconUrl,
    ...
    [selected, defaultIconUrl, candidates, uploadedUrls],
  );
  ...
  const onUpload = (file: File | null) => {
    ...
    startTransition(async () => {
      ...
      setUploadedUrls((current) => new Set(current).add(r.iconUrl!));
    ...
    <div style={{ display: "grid", gap: 10 }}>
      ...
      {mode === "select" ? (
        candidates.length > 0 ? (
          ...
          >
            {noneButton}
            <p className="fn-muted fn-text-sm" style={{ margin: 0, gridColumn: "1 / -1" }}>
