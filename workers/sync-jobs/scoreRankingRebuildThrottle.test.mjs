...
function buildEventsIndexPayload(events) {
  return {
    generated_at: T0,
    items: events,
    group_sections: [],
...
function createDb({ rows = [], batchChanges = 1, failAll = false, failReads = false } = {}) {
  let batchCalls = 0;
  return {
...
test("active events throttle: skip under 1h and enqueue after 1h", async () => {
  ...
  const env = {
    KV: kv,
    R2: createR2(
      new Map([[EVENTS_INDEX_R2_KEY, buildEventsIndexPayload([activeEvent({ start_time: now - 100, end_time: now + 100 })])]]),
    ),
    DB: createDb().db,
  };
...
test("inactive events throttle: skip under 3h and enqueue after 3h", async () => {
  ...
  const env = {
    KV: kv,
    R2: createR2(
      new Map([[EVENTS_INDEX_R2_KEY, buildEventsIndexPayload([endedEvent({ start_time: now - 200, end_time: now - 10 })])]]),
    ),
    DB: createDb().db,
  };
...
test("missing KV marker always enqueues score rebuild", async () => {
  ...
  const env = {
    KV: kv,
    R2: createR2(
      new Map([[EVENTS_INDEX_R2_KEY, buildEventsIndexPayload([endedEvent()])]]),
    ),
    DB: db,
  };
