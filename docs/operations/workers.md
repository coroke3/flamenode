...
3. `flamenode-background-jobs`へ`DISCORD_BOT_TOKEN`、`YOUTUBE_API_KEY`、`WORKER_ADMIN_TOKEN`をsecret設定する。
4. `npm run workers:deploy:verified`で新Workerをdeployする。
5. deploy成功後、scriptが旧3 Workerを削除する。
