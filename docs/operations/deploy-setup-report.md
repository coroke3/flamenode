...
| `flamenode-fast-jobs` | `DISCORD_BOT_TOKEN` 必須。加えて Forum 3種（`DISCORD_WEBHOOK_URL_FORUM_ACCOUNT` / `_EVENT` / `_SYSTEM`）すべて必須。legacy `DISCORD_WEBHOOK_URL` は任意（`webhook_target` 無しの旧 pending 行のみ） |
